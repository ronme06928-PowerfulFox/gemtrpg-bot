...
# 特殊パッシブ (SPECIAL PASSIVES) 定義マニュアル

特殊パッシブ（`Pa-XX`）は、**行動回数の増加**や**初期FPの増加**など、通常のスキルやバフとは異なる特殊なステータス補正を行うためのスキル群です。
主にスプレッドシートの「特殊パッシブ」シートで定義されます。

## ファイル構造

* **ファイル名**: `passives_cache.json` (キャッシュとして生成)

* **形式**: JSON Object (Key: Passive ID)

## フィールド定義

| フィールド | 型 | 必須 | 説明 | 備考 |
| :--- | :--- | :--- | :--- | :--- |
| `id` | string | ✅ | パッシブID | 例: "Pa-00" |
| `name` | string | ✅ | スキル名 | |
| `description` | string | ✅ | 効果説明文 | UI表示用 |
| `flavor` | string | | フレーバーテキスト | UI表示用 |
| `effect` | object | ✅ | 効果定義 | `stat_mods` を含むオブジェクト |

### Effect Object (Stat Mods)

キャラクターの基本ステータスに対して、戦闘開始時やステータス計算時に補正を加えます。
...
* **`stat_mods`**: ステータス名のキーと、加算する数値のペア。

### 使用可能なステータス (Stat Mods Keys)

| キー名 | 説明 | 効果 |
| :--- | :--- | :--- |
| **`行動回数`** | 1ラウンドあたりの行動回数 | デフォルト(1)に加算されます。<br>例: `1` なら計2回行動。 |
| **`初期FP`** | 戦闘開始時のFP | 戦闘開始時にこの値分のFPを得ます。 |
| **`速度`** | 基本速度 | イニシアチブ判定の基準値に加算されます。 |

## 定義例

### Pa-00: 迅速 (行動回数+1)

「1ラウンドに2回行動可能になる」パッシブスキルの定義例です。
...
```json
  "Pa-00": {
    ...
    "name": "迅速",
    "cost": 0,
    "description": "1ラウンドあたりの行動回数+1。",
    "flavor": "素早い思考は素早い行動を可能とする。",
    "effect": {
      "stat_mods": {
        "行動回数": 1
      }
...
戦闘開始時からFPを持っている状態にする定義例です。

```json
  "Pa-05": {
    "id": "Pa-05",
    "name": "熟練の構え",
    "description": "戦闘開始時、FPを3獲得する。",
    "effect": {
      "stat_mods": {
        "初期FP": 3
      }
