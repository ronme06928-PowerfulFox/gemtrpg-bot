...
| | `static/js/tab_visual_battle.js` | レンダリングキャッシュ、ログ制限、Socketリスナー初期化順序修正、**Local Override同期ロジック** |
...
### 【注意】高速移動時の同期ズレについて (Known Limitation)

キャラクターのトークンを**極めて短時間に連続して移動およびドロップアウト**を繰り返すと、サーバーとの通信ラグにより、手元の駒が一瞬だけ古い位置に引き戻されるように見える現象（ラバーバンディング）が発生することがあります。
これは通信の整合性を保つための仕様上の限界であり、通常のプレイ速度では発生しませんが、意図的に高速連打などで操作した場合には観測される可能性があります。プレイに支障がある場合は、少し間隔を空けて（0.5秒程度）操作してください。
