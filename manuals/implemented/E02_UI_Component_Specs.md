...
- モーダル本体（`openOwnedCharactersModal`）は以下を提供する:
  ...
  - 一覧表示: 各行は名前先頭1文字を表示する円形アバター（`avatarColorFor(name)`で名前から決定的に色を選ぶ）＋名前＋「残り経験値／累計exp_total／更新日時」。
  - 各行のアクション（絵文字アイコンボタン＋`title`属性でツールチップ）: 🌱「成長」（後述の軽量成長パネルを展開）、📝「編集」（`/chara_creator?owned_id=<id>`を新規タブで開く）、📤「JSON出力」（`{kind:"character", data:...}`形式でダウンロード）、🗑️「削除」（確認ダイアログ後に論理削除）。
  - 持ちキャラが0件の場合は空状態カード（🗡️アイコン＋「＋ 最初のキャラクターを作成」ボタン、ヘッダーの新規作成ボタンと同じ`/chara_creator`を開く）を表示する。
