...
- `lasting` は更新されず、最初に付与した `震盪` の継続ラウンドを維持する。
...
## 6. 2026-04 追補: 凝魔・蓄力（特殊バフ）

### 6.1 データ定義方針
- `凝魔` と `蓄力` は、状態異常のようにスタックするが、分類上はバフとして扱う特殊リソース。
- スタック値は `special_buffs[].count`（または `special_buffs[].data.count`）で管理する。
- `lasting` 未指定時は内部で `-1`（無期限）として扱う。

### 6.2 参照・消費
- `condition.param` で `<バフ名>_count` と `buff_count:<バフ名>` を利用可能。
- `delay > 0` の行はカウント対象外。
- `count` 未指定行は 1 として扱う。
