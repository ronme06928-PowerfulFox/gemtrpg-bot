...
**更新日**: 2026-06-28  
**種別**: implemented（P0全件実装確認済み。P1/P2は保守・任意扱い）  
...
- `manager/data_manager.py`、`scripts/check_schema.py`、`scripts/migrate_visibility.py`、`scripts/register_default_images.py` は当初 `from app import app` に依存していたが、2026-05-31に `GEMTRPG_DISABLE_DEFAULT_APP=1` + `create_app(run_startup=False, register_sockets=False)` へ置き換え済み。
...
- `manager/data_manager.py` と一部 `scripts/*.py` の `from app import app` 依存は、2026-05-31の後始末で解消済み。
...
6. **後始末: メンテナンス入口のfactory化**
   - 2026-05-31: `scripts/check_schema.py`、`scripts/migrate_visibility.py`、`scripts/register_default_images.py`、`manager/data_manager.py` の手動実行入口から `from app import app` を除去した。
   - 2026-05-31: 各メンテナンス入口は `GEMTRPG_DISABLE_DEFAULT_APP=1` を設定してから `create_app(run_startup=False, register_sockets=False)` を明示的に呼び、import時startupとSocket登録を避ける。

最初に触るべきファイル:

1. `app.py`: factory、設定、起動処理、HTTP登録の分離。
2. `wsgi.py`: 本番入口を新設。
3. `manager/data_manager.py`: `from app import app` の除去または遅延化。2026-05-31完了。
4. `scripts/*.py`: `from app import app` 依存の置換。2026-05-31完了。
...
- `python -m py_compile scripts/check_schema.py scripts/migrate_visibility.py scripts/register_default_images.py manager/data_manager.py app.py wsgi.py`: OK
- `rg -n "from app import app|from app import app, db" manager\data_manager.py scripts\check_schema.py scripts\migrate_visibility.py scripts\register_default_images.py`: no matches
- `python -c "import os; os.environ['GEMTRPG_DISABLE_DEFAULT_APP']='1'; import app; print(app.app is None)"`: `True`
- `python -c "import os; os.environ['GEMTRPG_SKIP_IMPORT_STARTUP']='1'; import app; print(app.app is not None, len(app.app.url_map._rules))"`: `True 34`
- `python -c "import os; os.environ['GEMTRPG_DISABLE_DEFAULT_APP']='1'; from app import create_app; a=create_app(run_startup=False, register_sockets=False); print(len(a.url_map._rules))"`: `34`
- `pytest -q --ignore=.claude`: 404 passed, 2 skipped
- `python scripts/check_text_encoding.py`: OK
- `python scripts/check_mojibake_markers.py`: OK
...
3. **window公開の縮小**
   ...
   - 2026-05-31: 調査実施。`window.*` は旧JS間の接着剤として広く使われており、一括削除は避ける。
   - 当面残す公開面:
     - 中核ブリッジ: `window.SocketClient`、`window.BattleStore`、`window.EventBus`、`window.battleState`、`window.socket`
     - セッション/権限: `window.currentUsername`、`window.currentUserId`、`window.currentUserAttribute`、`window.currentUserRole`、`window.currentUserIsAppAdmin`
     - マスターデータ: `window.allSkillData`、`window.radianceSkillData`、`window.allPassiveData`、`window.buffCatalogData`、`window.glossaryData`
     - 共通UI/整形: `window.showAppConfirm`、`window.showAppPrompt`、`window.Glossary`、`window.formatGlossaryMarkupToHTML`、`window.formatSkillDetailHTML`、`window.formatWideResult`
     - 既存コンポーネント橋渡し: `window.TimelineComponent`、`window.ActionDockComponent`、`window.DeclarePanelComponent`、`window.VisualMapComponent`、`window.MatchPanelComponent`、`window.ResolveFlowPanelComponent`
   - 内部化候補:
     - `window.GRID_SIZE`、`window.FIELD_SIZE`、`window.TOKEN_OFFSET` などの定数は `Constants.js` と `legacy_globals.js` で重複しているため、最終的には一方へ寄せる。
     - `window.visualScale`、`window.visualOffsetX/Y`、`window.visualMapHandlers`、`window._dragBlockClick`、`window._localCharPositions`、`window._lastSentMoveTS` などのVisual Map状態は `MapState` またはVisual Map専用モジュールへ寄せる。
     - `window._lastLogCount`、`window.actionDockInitialized`、`window._matchPanelAutoExpanded`、`window._charStatUpdatedListenerRegistered` などのUI制御フラグは、それぞれのコンポーネント内状態へ移す。
     - `window.openDuelModal`、`window.closeMatchPanel`、`window.sendSkillDeclaration`、`window.executeMatch` などのMatchPanel系関数は、`MatchPanel` / `MatchPanelState` 側へ段階移行する。
     - `window.openSyncedWideMatchModal`、`window.populateWideMatchPanel`、`window.resetWideMatchState`、`window.updateWideExecuteButtonState` などの広域マッチ系関数は、専用名前空間またはモジュールへまとめる。
     - `window.quickEditItemData` / `window.quickEditItemDataPromise` は `action_dock.js` 内のモジュールローカルキャッシュへ移せる。
     - `window.socket.emit` を直接使う箇所は、既存の `SocketClient` outbound APIへ順次移す。
   - 優先順:
     - 低リスク: ローカルキャッシュ、UI制御フラグ、定数重複の整理。
     - 中リスク: Visual Map状態の `MapState` 集約。
     - 高リスク: MatchPanel / WideMatch / Duel操作関数の内部化。HTML inline handlerや旧JSからの参照が残るため、置換範囲の確認が必要。
4. **SocketClient責務整理**
   - `SocketClient` 内の処理を、Socket lifecycle、core listener、legacy互換購読API、outbound command APIに分ける。
   - 2026-05-31: `_ensureSocket()`、`_getRoomName()`、`_bindCore()`、`_emit()`、`_normalizeIntentTarget()` を追加し、初期化確認、room名解決、core listener登録、emit、intent target正規化を分離した。
   - 2026-05-31: 送信APIのうち `moveToken()`、`declareSkill()`、`startMatch()`、`sendIntentUncommit()`、`sendResolveStart()`、`sendResolveFlowAdvance()`、`sendRoundEnd()` は共通helper経由に整理した。
   - 2026-05-31: connected状態をログに含める `sendIntentPreview()`、`sendIntentCommit()`、`sendResolveConfirm()` は挙動維持を優先し、直接emitを残しつつSocket取得とtarget正規化のみ共通化した。

現行スコープの完了判断:

- `SocketClient.on()` / `SocketClient.off()` により、低リスクなSocket購読境界は追加済み。
- `state_updated` の直接購読は `SocketClient` 本体へ集約済み。
- `SocketClient` の主要責務は helper 単位へ分離済み。
- `window.*` の一括移行は、既存HTML inline handlerと旧JS間参照への影響が大きいため、IMP-25本体では実装対象外とする。
- MatchPanel / WideMatch / Duel 周辺の内部化は、別番号の将来タスクとして扱う。
...
- `SocketClient.js`: lifecycle / core listener / compatibility subscription / outbound command の区分へ整理済み
...
- 2026-05-31: `requirements.txt` の未固定だった直接本番依存をローカル環境で確認したバージョンに固定した。既に固定済みだった `gunicorn==22.0.0` は既存値を維持した。
- 2026-05-31: `requests==2.31.0` を本番依存へ明示追加した。各loaderが直接 `requests` をimportしているため。
- 2026-05-31: `pytest` / `pytest-cov` は `requirements-dev.txt` に分離した。開発環境では `pip install -r requirements-dev.txt` を使う。
...
- `python - <<metadata check>>`: 直接依存のインストール済みバージョンを確認
- `requirements.txt`: 本番直接依存を固定
- `requirements-dev.txt`: `-r requirements.txt` + テスト依存を固定
...
ARCH-001/ARCH-002完了後は、P1の残タスクへ直行せず、P2のうち負担が軽く影響範囲を限定しやすい項目を先に処理した。
...
2026-05-31時点で、上記順番の作業は現行スコープまで完了済み。
...
**状況**: IMP-25実装時に確認済み（2026-05-31）

- [x] 対象番号をPR本文、コミットメッセージ、テスト名のいずれかに含める。
- [x] 既存の日本語キー、英語キー、旧JSON定義の互換性を確認する。
- [x] Socketイベント名を変更しない。
- [x] GM権限を扱う変更では、非GMの拒否テストを追加する。
- [x] GM PINは4桁数字、マスターキーは8桁数字として検証する。
- [x] GM PINを平文でDB、ログ、Socket payload、画面再表示に残さない。
- [x] `innerHTML` を使う場合は、入力元が信頼済みか、サニタイズ済みかを明記する。
- [x] 巨大ファイルへ追記する場合は、分割できない理由をPRに書く。
...
- [x] `IMP-25-P0-*` がすべて完了している。
- [x] 通常pytestでネットワーク依存e2eが走らない。
- [x] `python scripts/check_text_encoding.py` が通る。
- [x] `python scripts/check_mojibake_markers.py` が通る。

推奨完了条件:

- [x] `pytest -q --ignore=.claude` が通る。
- [x] `pytest -q tests/test_python_module_size_guard.py` が通る。
- [x] 新規コードが `manuals/module_size_policy.md` に違反しない。

---

## 7. 未決事項

現時点で、IMP-25を閉じるために必ず決めるべき未決事項はない。
...
- `IMP-26-FE-001`: `window.*` 互換公開面の段階的な内部化。
- `IMP-26-FE-002`: MatchPanel / WideMatch / Duel 操作関数の専用モジュール化。
- `IMP-26-SEC-001`: GM PIN失敗回数制限または監査ログ強化。
