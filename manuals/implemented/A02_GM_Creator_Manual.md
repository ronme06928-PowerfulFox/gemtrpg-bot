...
GM権限は、ルーム単位のGM権限と、アプリ全体のユーザー管理権限に分離されています。

### 1.1 ルームGM権限

ルームGM権限は、そのルーム内の進行・代理操作・GM専用操作に使います。

- ルーム作成時、作成者は4桁数字のGM PINを手入力します。
- ルームへ入室する際は、`プレイヤー` または `GM` を選択します。
- GMとして入室する場合は、通常はそのルームの4桁GM PIN、または8桁マスターキーを入力します。
- アプリ管理者権限を持つユーザーは、GM PIN入力なしで任意ルームへGMとして入室できます。
- ルームからロビーへ戻ると、ルームGM状態は解除されます。

クライアントから送信された `attribute=GM` は、サーバー側の認証結果なしでは信用されません。

### 1.2 アプリ管理者権限

アプリ管理者権限は、ユーザー管理画面の削除・所有権譲渡・管理者付与/解除に使う権限です。ルームGM権限とは別です。

- ユーザー管理画面の一覧/詳細は、ログイン済みユーザーなら閲覧できます。
- ユーザー削除・所有権譲渡は、アプリ管理者、または操作時に8桁マスターキーを入力した場合だけ実行できます。
- ユーザー管理画面から、8桁マスターキーを入力して特定ユーザーへアプリ管理者権限を付与/解除できます。
- 任意ルームでGMとして入室しても、アプリ管理者権限は付与されません。

### 1.3 マスターキー

マスターキーは8桁数字で、ローカルでは `.env`、Renderでは環境変数 `GM_MASTER_KEY` に設定します。

- 未設定の場合、マスターキー機能は無効です。
- マスターキーはルームGM入室、ルーム削除、ユーザー管理の管理操作、アプリ管理者権限の付与/解除に使えます。
- `SECRET_KEY` とは別の値です。
