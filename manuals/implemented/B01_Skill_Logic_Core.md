...
- `戦慄殺到`
- `荊棘飛散` — 対象の荊棘・荊棘重絡を全消費し、`ceil(荊棘 × 荊棘重絡 ÷ 敵人数)` を同陣営全員に分配（荊棘重絡 = 0 なら `ceil(荊棘 ÷ 敵人数)`）
- `DRAIN_HP`
...
- `HIT` / `AFTER_DAMAGE_APPLY` で新たに付与された亀裂は、その付与を発生させた同じダメージには乗らず、次にスキルでダメージを受ける時から有効です。
- Select/Resolve では `END_MATCH` / `WIN` / `LOSE` がスキルダメージ判定より前に発火するため、これらの timing で付与された亀裂は同じマッチのスキルダメージに反映されます。
...
荊棘はスキル使用後に消滅する。消滅時の挙動はスキルの分類で決まる。

| 分類 | ダメージ | 荊棘 |
|---|---|---|
| `物理` / `魔法` / `補助` | 荊棘値ぶんの HP 自傷（`DamageSource.THORNS`） | 0 に |
| `防御` / `回避` | なし | 0 に |

判定に使う定数（`manager/constants.py`）:
...
処理は通常マッチ（`duel_solver.py`）・広域戦闘（`wide_solver.py`）の両方で行われる。  
ラウンド終了時の自動減衰は**ない**（スキル使用時消滅に一本化）。

Select/Resolve では、荊棘処理はマッチ結果確定後の `END_MATCH` / `WIN` / `LOSE` の後、スキルダメージ判定用の状態参照より前に行う。したがって、`END_MATCH` / `WIN` / `LOSE` で付与された荊棘重絡は同じマッチの荊棘消滅を肩代わりできるが、`HIT` で付与された荊棘重絡はそのマッチの荊棘消滅には間に合わない。

#### 荊棘重絡（Bu-50）の肩代わり

荊棘が消滅しようとするとき、先に荊棘重絡のスタックを確認する。

1. 荊棘重絡 > 0 → 荊棘重絡を 1 消費し、荊棘はそのまま残る
2. 荊棘重絡 = 0 → 荊棘が消滅する

**荊棘によるダメージは荊棘重絡の有無に関わらず発生する**（消滅の肩代わりのみ）。

#### 荊棘重絡の付与

荊棘重絡は `states[]` に汎用スタック状態として格納される。  
スキルからは `APPLY_BUFF` タイプで Bu-50 を使って付与する。

`apply_buff()` 内の Bu-50 ハンドラが `states["荊棘重絡"]` へ直接加算するため、  
消費コード（`get_status_value` / `_update_char_stat`）と整合する。

スプレッドシートの `特記処理` への記述例:
...
対象の荊棘・荊棘重絡を全消費し、対象と同陣営の生存・配置済みキャラ全員に等分ダメージを与える。

```
荊棘重絡 > 0: ダメージ = ceil(荊棘 × 荊棘重絡 ÷ 敵人数)
荊棘重絡 = 0: ダメージ = ceil(荊棘 ÷ 敵人数)
```

陣営判定は `char.type / team / side / faction` を参照する（`ally` / `enemy` 系）。  
ダメージは `CUSTOM_DAMAGE` 変更タプルとして各キャラに発行される。
...
- `RESOLVE_END`: 解決フェーズ終了時（Resolve完了直前）、コミット済みスロットごと
...
6. `LOSE`（防御側）
7. 荊棘処理（攻撃側）
8. スキルダメージ判定用の状態参照（亀裂など）
9. `UNOPPOSED`
10. `HIT`
11. ダメージ反映
12. `AFTER_DAMAGE_APPLY`（攻防）

`END_MATCH` / `WIN` / `LOSE` で付与された亀裂は 8 の参照に間に合う。`HIT` で付与された亀裂は、このマッチのスキルダメージには乗らない。`HIT` の `CUSTOM_DAMAGE` など効果自体は従来どおり発火するが、状態参照の基準時点は 8 とする。
...
### 13.5 実装上の注意点（2026-06-28 検証済み）

**skills_cache.json のキー形式**

本番の `data/cache/skills_cache.json` は Google Sheets 由来の日本語キーを使用する。
`build_skill_reference` は英語キーを優先し、見つからない場合に日本語キーへフォールバックする:

| 参照目的 | 英語キー | 日本語キー（フォールバック） |
|---|---|---|
| カテゴリ判定 | `category` | `分類` |
| 距離判定 | `distance` | `距離` |
| 属性判定 | `attribute` | `属性` |

英語キーのみで実装すると、実スキルデータに対してカテゴリ/距離封印が機能しない。

**`_apply_cost` の消費優先順位**（`manager/battle/resolve_effect_runtime.py`）

1. `skill.get("effective_cost")` が存在する → そのまま消費（commit 時の評価値を正とみなす）
2. なければ `get_effective_skill_cost(actor, skill_id)` を呼ぶ（battle_state なし・actor 制約のみ反映）
3. それも空なら `_extract_skill_cost_entries(skill)` でスキル本来のコストを抽出

**field-effect add_cost の消費保証**

`_apply_cost` の fallback パス（2番目以降）は `battle_state` を渡さないため、
field_effects 由来の add_cost はコスト評価時には反映されない。

対応策: `resolve_auto_single_phase.py` が intent の `effective_cost`（commit 時に格納済み）を
outcome の skill dict に埋め込み、`_apply_cost` が優先パス（1番目）で消費できるようにする。
...
mass フェーズ（`resolve_auto_mass_phase.py`）は `apply_cost: False` 固定のため変更不要。
...
## 2026-07 追補: process_skill_effects の実装構成（effect_handlers 分割）

計画書29（2026-07-07 実装完了・削除済み）により、`manager/game_logic.py` の
`process_skill_effects`（旧・単一関数約1120行）を効果タイプ別ハンドラへ分割した。
**公開API・効果の挙動・ログ文字列・changes_to_apply の形式は分割前と同一**
（全既存テスト無修正で通過。薄い分岐は `tests/test_thin_effect_branches.py` で特性化済み）。

### 構成

```text
manager/battle/effect_handlers/
├── __init__.py        # EFFECT_HANDLERS: dict[effect_type, handler] の統合
├── session.py         # EffectSession（1回の process_skill_effects の共有状態）
├── state_effects.py   # APPLY_STATE / APPLY_STATE_PER_N / MULTIPLY_STATE / APPLY_FISSURE_BUFFED
├── buff_effects.py    # APPLY_BUFF / APPLY_BUFF_PER_N / REMOVE_BUFF
├── stack_resources.py # CONVERT_STACK_RESOURCE_VARIANT / CONSUME_BUFF_COUNT_FOR_GAIN / _FOR_POWER
├── action_effects.py  # GRANT_SKILL / USE_SKILL_AGAIN / SUMMON_CHARACTER
└── power_effects.py   # DAMAGE_BONUS / MODIFY_ROLL / FORCE_UNOPPOSED / MODIFY_BASE_POWER / MODIFY_FINAL_POWER / DRAIN_HP
```

### 処理の流れ

1. `game_logic.process_skill_effects` が `EffectSession` を生成
   （`total_bonus_damage` / `log_snippets` / `changes_to_apply` / `simulated_chars` /
   状態取得・設定関数の注入を保持）。
2. 前処理（出身ボーナス・ステージ効果・蓄力誘爆・`repeat_count` 展開）と
   対象解決（`target_select=RANDOM` / self / target / ALL_* / NEXT_ALLY）、`condition` 判定は
   従来どおり `game_logic` 側で行う。
3. 効果タイプごとに `EFFECT_HANDLERS[effect_type](effect, target_obj, sim_target, session)` を呼ぶ。
4. `CUSTOM_EFFECT` のみ、plugins の `EFFECT_REGISTRY` 連携（sim→実体の付け替えあり）のため
   `game_logic` 側の分岐に残している。

### 規約

- ハンドラ署名は `handle_xxx(effect, target_obj, sim_target, session) -> None`。
  結果は `session` に積む（戻り値は使わない）。
- ハンドラモジュールは `game_logic` をトップレベル import しない（循環回避）。
  ラッパ関数が必要な場合は関数内で遅延 import する。
- 新しい効果タイプを追加する場合は、該当グループのモジュールへ関数を追加し、
  そのモジュールの `HANDLERS` 辞書に登録する（`__init__.py` が自動統合する）。
