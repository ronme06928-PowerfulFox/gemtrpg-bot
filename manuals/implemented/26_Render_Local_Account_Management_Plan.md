...
**完了日**: 2026-06-28（Phase 0〜8 実装完了・Render 本番投入済み・migration fail-fast 実装）  
**種別**: implemented  
**状態**: 実装完了
...
- [x] **第3弾 ルーム情報UI**: ロビーのowner/gmカードに「⚙️設定」追加。`openRoomSettingsModal` で説明・募集状態・公開設定の表示/編集（owner=全項目、gm=募集のみ）、参加コード設定/再発行（PIN指定可）/失効。`update_settings`・`set_join_code`・`clear_join_code` 配線。プレビュー検証OK。
- [x] **第4弾 ユーザー設定UI**: 表示名変更を `/api/change_display_name` へ。パスワード変更モーダル（現パスワード→新パスワード）。`set_account_password` に現パスワード再確認を必須化（誤り/無しは403）。session再開時の復旧コードモーダルを非ブロッキング化（初回発行時のみ）。プレビュー検証OK（現パス誤り403/正200、新パスログイン可・旧不可）。

**Phase 7 完了。** UI連携のブラウザE2E・実クライアントSocket確認まで実施済み。次はPhase 8 contract（旧導線撤去）→ 運用マニュアル → main合流。
...
## 17. 実装開始前チェックリスト（全完了・2026-06-28）

- [x] Q26-001〜Q26-004、およびQ26-014（オリジン構成）・Q26-015（auth_version遷移）を決定事項ログへ記録した。
- [x] Phase 0の拒否テスト一覧を先に作成した。（`tests/test_phase0_auth_boundary.py`）
- [x] `/save_room` のデスクトップ側呼び出し元を棚卸しした（mobileは未使用を確認済み）。
- [x] トークン再発行停止による既存ユーザーへの影響を確認した。
- [x] 全HTTP routeとSocket eventの必要role一覧を作成した。（§8.2・§8.3 と Phase 5 棚卸し）
- [x] 既存DBの重複表示名、owner不在ルーム、所有者不明キャラクターをdry-run集計した。（クリーンスタート方式採用により backfill 不要と確定）
- [x] Render DBバックアップとrollback担当を確認した。
- [x] mobileを同時対応するか、一時停止するか決定した。（Q26-012: 開発停止・`/mobile` 404 停止）
- [x] frontend変更を含むPRで `npm run build` を実行する手順を明記した。（AGENTS.md）
