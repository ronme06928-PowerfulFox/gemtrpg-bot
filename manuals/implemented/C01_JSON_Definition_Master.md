...
## 12. 2026-07 追補: スキルカタログ lint / 相場自動集計ツール

計画書31（2026-07-08 実装完了・削除済み）により、スキルJSON定義（`特記処理`＝JSON rule v2）のスキーマ検証と、`F02_Battle_Balance_Designer_Skill_Manual.md` の相場表自動生成を行う統合CLIを整備した。
...
```bash
python scripts/skill_catalog_tool.py lint                    # スキーマ検証 + 相場逸脱WARN
python scripts/skill_catalog_tool.py lint --json              # JSON形式で出力
python scripts/skill_catalog_tool.py build-market-rate         # F02のB群相場表を再生成
python scripts/skill_catalog_tool.py build-market-rate --check # 差分があれば exit 1（CI向け）
```
...
`manager/data_manager.py::update_all_data()` は、スキルデータ取得（Google Sheets同期）に成功した直後に `lint_catalog()`（ERROR検査のみ、`warn_catalog()` は呼ばない）を実行する。ERRORが1件でもあれば `--update` 全体を失敗（exit 1）として扱う。これにより、壊れたスキル定義がキャッシュに残ったまま気づかれない状態を防ぐ。CI（`.github/workflows/skill-smoke.yml`）も同様に `lint`（`--warn` なし）と `build-market-rate --check` を実行し、リポジトリにマージされる前の安全網としている。相場逸脱WARNはどちらのパイプラインにも含まれない。
