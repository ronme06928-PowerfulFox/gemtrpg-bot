...
| 28 | P0-1 セッションログの長期保全方式決定＋エクスポート/検索 | 高 | 500件切り捨て問題の実害緩和。保全方式は§7で先に決める |
...
utils/common_routes分割完了を反映済み。**§7未決定事項が7件あり、実装前に一問一答での確定が必要**:
ログ長期保全方式 / undoの範囲 / サーバーダイス仕様の正本化 / スキル検索の去就 /
ダークモードの粒度 / P3着手要否 / 04残課題との統合。

推奨実装順（Phase1〜5、独立度高い順）:
1. 保存信頼性 → ログ長期保全方式の決定 → ログエクスポート/検索
