...
**作成日**: 2026-07-07（最終更新: 2026-07-08 — 計画30/31/32/33/34を追加。31・32・33・34は実装完了・削除済み。未計画項目ゼロ）
...
| 28 | P0-1 セッションログの別テーブルアーカイブ＋エクスポート/検索 | 高 | 500件切り捨て問題の実害緩和。保全方式は別テーブルアーカイブに決定済み |
...
P0〜P3の改善提案16項目（G1〜G16のギャップに対応）。2026-07-08の再調査で、
`/roll` `/sroll` のサーバーダイス化、旧テキスト戦闘/旧wide match系JS削除、
utils/common_routes分割完了を反映済み。ログ長期保全方式は2026-07-08に
**別テーブルアーカイブ**で決定済み。**§7未決定事項が6件あり、実装前に一問一答での確定が必要**:
undoの範囲 / サーバーダイス仕様の正本化 / スキル検索の去就 /
ダークモードの粒度 / P3着手要否 / 04残課題との統合。

推奨実装順（Phase1〜5、独立度高い順）:
1. 保存信頼性 → ログ別テーブルアーカイブ → ログエクスポート/検索
2. サーバーダイスの回帰テスト・仕様反映（実装済み挙動の固定）
3. 手番通知 + キーボードショートカット（フロントのみ、並行可）
4. イニシアチブ手動調整 → 限定undo（§7方式決定が前提）
5. ダークモード / スキル検索 / aria
6. 残骸ファイル整理（随時）
...
2. **§7の一問一答から始める**: 28（プレイ体験改善）、30（シミュレータ）
3. **戦闘メモリ設計が要る**: 12（逆襲）
4. 31・32・33・34は実装完了。残る計画書は04・12・13・16・24・28・30の7本。
