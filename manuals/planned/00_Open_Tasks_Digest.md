...
**作成日**: 2026-07-07（最終更新: 2026-07-20 — 計画37 PR 1「キャラクタータグ基盤」を実装）
**位置づけ**: `manuals/planned/` 配下の各計画書に散っている未完了タスクを1本にまとめた索引。
使用量が厳しい時などに、個別ファイルを開かずここだけ見て次の一手を判断するためのもの。
**このファイルは要約であり正本ではない。** 実装時は必ず該当する個別計画書（04/12/13/16/24/28/30/35/37）を読むこと。
...
| 37 | キャラクタータグ・条件判定・シナリオギミック拡張 | PR 1実装済み、PR 2以降未着手 | `tag_ids` / `disabled_tag_ids`と出身・輝化・GMタグは実装済み。文字列condition、最大HPダメージ、死亡原因伝播、PvE順序付き対象候補を段階実装する |
...
### 37_Character_Tags_Conditional_Damage_And_PvE_Targeting_Plan.md（キャラクタータグ・条件判定・シナリオギミック拡張）

PR 1は実装済み。キャラクターへ`tag_ids` / `disabled_tag_ids`を追加し、プレイヤーは出身国と輝化スキルから
...
キャラクター・スキルの死亡時パッシブへの伝播を追加する。PvE行動フローチャートは順序付き対象候補へ拡張し、
「同陣営の`種別:瓦礫`を優先し、不在ならPCを狙う」をGM操作なしで解決する。
「鉄塊を捥ぎり喰う」は`target_scope:any`、3/6/9R周期、瓦礫条件付き最大HPダメージとして結合確認する。
...
3. **戦闘メモリ設計が要る**: 12（逆襲）
4. **シナリオギミック基盤**: 37のPhase 1（タグ基盤）は実装済み。次はPR 2（文字列condition・最大HPダメージ）へ進める。
5. 28・31・32・33・34・36は実装完了/クローズ。残る計画書は04・12・13・16・24・30・35・37の8本。
6. **本命**: 35（ホロウ）のPhase 0から着手可能。前提の36（キャラ管理）は実装完了済みなので、味方キャラの持ち込み（持ちキャラ投入）はそのまま使える。35の残論点は準備作業・技術検証3件のみ。
