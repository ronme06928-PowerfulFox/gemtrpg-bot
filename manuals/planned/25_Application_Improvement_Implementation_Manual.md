...
- `app.py` はグローバルに `app = Flask(__name__, static_folder=None)` を作成している。
- import時に `WhiteNoise`、`cloudinary.config()`、`CORS(app)`、`Compress(app)`、`db.init_app(app)`、`socketio.init_app(app)` が実行される。
- import時に `run_auto_migration(app)`、`db.create_all()`、`buff_registry.auto_discover()`、`init_app_data()`、`read_saved_rooms_with_owners()` が実行される。
- Socketイベントは `import events.socket_main` などの副作用で `@socketio.on` 登録される。
- HTTPルートは `@app.route` 直書きで約30個あり、factoryへ移すには登録方式の整理が必要。
...
- `app.py` をimportするだけで本番DBへ接続し、migrationや初期データ読み込みが走る。
- テスト用に軽量なFlask appを作りづらく、APIテストが本番設定や外部環境に近づきやすい。
- 起動時処理とアプリ定義が混ざっており、Render起動・ローカル起動・スクリプト実行・テスト実行の責務が分かれていない。
- Socketイベント登録がimport順に依存しているため、factory化時に未登録または二重登録が起きるリスクがある。
- 既存の `app:app` 入口を壊すとRender起動に直撃する。
