...
| Phase | 内容 | 依存 |
|---|---|---|
| Phase 1 | P0-3 保存信頼性 → P0-1 ログ別テーブルアーカイブ＋エクスポート/検索 | ログ長期保全方式は「別テーブルへアーカイブ」で決定済み（§8） |
...
2. ログ別テーブルアーカイブ（DB/モデル/移行）＋ログエクスポートAPI
3. ログ検索UI
...
| 日付 | 論点 | 決定 | 根拠 |
|---|---|---|---|
| 2026-07-08 | ログの長期保全方式 | **切り捨て前に別テーブルへアーカイブする** | 手動エクスポートだけでは500件超過後の過去ログ喪失を防げない。TRPG用途ではセッション後のログ参照・整形価値が高いため、まず古いログをDBへ保全する。 |
