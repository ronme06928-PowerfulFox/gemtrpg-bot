...
# 亀裂ラウンド/加速減速スタックの解決ヘルパーと apply_buff 本体は
# manager/buff_apply.py へ移設済み（計画書33 Phase 1）。
# この3ヘルパーは apply_buff 内でのみ使われていたため utils.py 側の re-export は不要。
# apply_buff は `from manager.utils import apply_buff` という既存の呼び出し全てを
# 無改変で保つために re-export する。
from manager.buff_apply import apply_buff
...
def apply_dice_power_bonus_to_command(command, dice_power_bonus):
    from manager.battle.dice_command import apply_dice_power_bonus_to_command as _apply
    return _apply(command, dice_power_bonus)
