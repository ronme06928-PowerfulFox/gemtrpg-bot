...
<div class="modal-content bo-modal bo-modal--enemy-formation">
  ...
  <div class="bo-layout bo-layout--catalog">
    <section class="bo-card">
      ...
      <details class="bo-field"><summary class="bo-field-label bo-list-main--clickable">上級者向けJSON編集</summary>
  ...
  <div class="bo-footer-actions"><button id="bo-sp-close-btn" class="bo-btn bo-btn--neutral">閉じる</button></div>
        ...
        const refreshRuleSummary = () => Array.from(el.rules.querySelectorAll('.bo-sp-rule-row')).forEach((row, idx) => {
            ...
            const dn = row.querySelector('.bo-sp-rule-display-name').value.trim();
            row.querySelector('.bo-sp-rule-summary-text').textContent = dn
                ? `効果ルール ${idx + 1}: ${dn}`
                : `効果ルール ${idx + 1}: ${t} / ${sc} / 値 ${row.querySelector('.bo-sp-rule-value').value || 0}`;
        });
        const addRuleRow = (r = {}, open = true) => {
            ...
            const d = document.createElement('details'); d.className = 'bo-sp-rule-row bo-sp-rule-card'; if (open) d.open = true;
