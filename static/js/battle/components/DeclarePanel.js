...
class DeclarePanel {
    ...
    _resolvePowerAdjustRows(calc) {
        ...
        const pb = (calc?.power_breakdown && typeof calc.power_breakdown === 'object')
            ? calc.power_breakdown
            : {};
        let baseMod = Number(calc?.skill_details?.base_power_mod || 0);
    ...
    _normalizeTargetScope(scope) {
        ...
        if (['self', 'self_only', 'caster', '自分', '自分対象', '自身', '自己対象'].includes(s)) return 'self';
    ...
    _inferTargetScopeFromSkill(skillId) {
        ...
        for (const raw of candidates) {
            ...
            if (!text) continue;
            if (['self', 'self_only', 'caster', '自分', '自分対象', '自身', '自己対象'].includes(text)) return 'self';
        ...
        if (tags.some((t) => ['self_target', 'target_self', '自分対象', '自身対象', '自己対象'].includes(t))) return 'self';
